...
const PathTikTok = () => (
    <>
        <rect x="0" y="0" width="20" height="20" rx="4" fill="#010101" />
        <path d="M13.5 2.5h-2v9.2a1.7 1.7 0 0 1-1.7 1.7 1.7 1.7 0 0 1-1.7-1.7 1.7 1.7 0 0 1 1.7-1.7c.16 0 .32.02.47.06V7.8c-.15-.01-.3-.02-.47-.02A4.8 4.8 0 0 0 5 12.6a4.8 4.8 0 0 0 4.8 4.8 4.8 4.8 0 0 0 4.8-4.8V7.2c.66.47 1.45.76 2.3.82V5.8a3.3 3.3 0 0 1-3.4-3.3z" fill="white" />
    </>
);

/* ─── Fixed SVG viewBox automation flow ───────────────────── */
// Each node group uses <svg> nested element with x/y/width/height
// This is more reliable than <g transform="translate(...)"> with React icons
const PATHS = {
...
/* Icon slot: renders as <svg x y width height> so embedded SVGs just work */
...
/* Floating particle along an SVG path via CSS animation */
...
const AutomationFlowSVG = () => (
    ...
    >
        <defs>
            ...
            {/* Center gradient */}
            <radialGradient id="cGrad" cx="50%" cy="35%" r="60%">
                <stop offset="0%" stopColor="#1f6b52" />
                <stop offset="100%" stopColor="#0b2419" />
            </radialGradient>
            {/* Path gradient L→center */}
            <linearGradient id="lgL" gradientUnits="userSpaceOnUse" x1="80" y1="170" x2="400" y2="170">
                <stop offset="0%" stopColor="#10b981" stopOpacity="0.1" />
                <stop offset="100%" stopColor="#10b981" stopOpacity="0.55" />
            </linearGradient>
            {/* Path gradient center→R */}
            <linearGradient id="lgR" gradientUnits="userSpaceOnUse" x1="400" y1="170" x2="720" y2="170">
                <stop offset="0%" stopColor="#10b981" stopOpacity="0.55" />
                <stop offset="100%" stopColor="#10b981" stopOpacity="0.1" />
            </linearGradient>
            {/* Named hidden paths for animateMotion */}
            {Object.entries(PATHS).map(([id, d]) => <path key={id} id={id} d={d} />)}
        </defs>

        {/* ── Connector lines (animated draw-in) ──────────── */}
        {/* Left */}
        {[
        ...
        {/* ── LEFT NODES ─────────────────────────────────── */}
        {/* Card backdrop helper */}
        {[74, 170, 266].map((cy, i) => (
            <motion.g key={`lc${i}`}
                initial={{ opacity: 0, x: -14 }} whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.05 * i }}>
                <rect x="52" y={cy - 28} width="96" height="56" rx="14" fill="white" fillOpacity={0.92} />
        ...
        <motion.g initial={{ opacity: 0, x: -14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
            ...
            <text x="110" y="86" fontSize="10" fill="#374151" fontWeight="600" textAnchor="middle" style={{ fontFamily: 'system-ui' }}>LinkedIn</text>
        </motion.g>
        {/* Facebook */}
        <motion.g initial={{ opacity: 0, x: -14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.18 }}>
            <SvgIcon x={82} y={158}><PathFacebook /></SvgIcon>
            <text x="110" y="182" fontSize="10" fill="#374151" fontWeight="600" textAnchor="middle" style={{ fontFamily: 'system-ui' }}>Facebook</text>
        </motion.g>
        {/* Google */}
        <motion.g initial={{ opacity: 0, x: -14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.26 }}>
            <SvgIcon x={82} y={254}><PathGoogle /></SvgIcon>
            <text x="110" y="278" fontSize="10" fill="#374151" fontWeight="600" textAnchor="middle" style={{ fontFamily: 'system-ui' }}>Google Ads</text>
        </motion.g>

        {/* ── CENTER NODE (SolveClin) ─────────────────────── */}
        <motion.g
        ...
        >
            {/* Animated outer ring */}
            <circle cx="400" cy="170" r="65" fill="none" stroke="#10b981" strokeWidth="1" opacity="0.18">
                <animate attributeName="r" values="65;75;65" dur="3.2s" repeatCount="indefinite" />
                <animate attributeName="opacity" values="0.18;0;0.18" dur="3.2s" repeatCount="indefinite" />
            </circle>
            {/* Second static ring */}
            <circle cx="400" cy="170" r="56" fill="url(#cGrad)" stroke="rgba(16,185,129,0.35)" strokeWidth="1.5" />
            {/* Shine */}
            <ellipse cx="393" cy="150" rx="22" ry="9" fill="white" fillOpacity={0.07} />
            {/* Text */}
            <text x="400" y="163" fontSize="12" fill="rgba(255,255,255,0.9)" fontWeight="800" textAnchor="middle" letterSpacing="0.08em" style={{ fontFamily: 'system-ui' }}>SOLVE</text>
            <text x="400" y="180" fontSize="12" fill="#34d399" fontWeight="800" textAnchor="middle" letterSpacing="0.08em" style={{ fontFamily: 'system-ui' }}>CLIN</text>
            {/* Small dot indicator */}
            <circle cx="400" cy="193" r="3" fill="#10b981" opacity={0.7}>
        ...
        {[74, 170, 266].map((cy, i) => (
            <motion.g key={`rc${i}`}
                ...
                <rect x="640" y={cy - 28} width="108" height="56" rx="14" fill="none"
                    stroke={i === 2 ? 'rgba(16,185,129,0.4)' : 'rgba(16,185,129,0.15)'}
        ...
        <motion.g initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.7 }}>
            ...
            <text x="708" y="86" fontSize="10" fill="#374151" fontWeight="600" textAnchor="middle" style={{ fontFamily: 'system-ui' }}>WhatsApp</text>
        </motion.g>
        {/* TikTok */}
        <motion.g initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.8 }}>
            <SvgIcon x={670} y={158}><PathTikTok /></SvgIcon>
            <text x="708" y="182" fontSize="10" fill="#374151" fontWeight="600" textAnchor="middle" style={{ fontFamily: 'system-ui' }}>TikTok</text>
        </motion.g>
        {/* CRM / Agenda */}
        <motion.g initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.9 }}>
            <text x="694" y="264" fontSize="9.5" fill="rgba(52,211,153,0.9)" fontWeight="700" textAnchor="middle" letterSpacing="0.06em" style={{ fontFamily: 'system-ui' }}>AGENDA</text>
            <text x="694" y="278" fontSize="9.5" fill="rgba(255,255,255,0.5)" fontWeight="500" textAnchor="middle" style={{ fontFamily: 'system-ui' }}>+ CRM</text>
        </motion.g>

        {/* ── Column labels ──────────────────────────────── */}
        <text x="100" y="26" fontSize="9.5" fill="rgba(107,114,128,0.7)" textAnchor="middle" letterSpacing="0.1em" fontWeight="600" style={{ fontFamily: 'system-ui' }}>LEADS</text>
        <text x="694" y="26" fontSize="9.5" fill="rgba(16,185,129,0.75)" textAnchor="middle" letterSpacing="0.1em" fontWeight="600" style={{ fontFamily: 'system-ui' }}>VENDAS</text>
    </svg>
);
...
export default function Integrations() {
    return (
        <section className={styles.wrapper}>
            {/* gradient bridge from dark (#172f25) → light */}
